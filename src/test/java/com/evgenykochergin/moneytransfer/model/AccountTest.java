...
public class AccountTest {
    ...
    private final Account testAcccount = Account.builder()
            .id(UUID.randomUUID())
    ...
    @Test
    public void deposit() {
        Account depositedAccount = testAcccount.deposit(Amount.of(new BigDecimal(1)));
    ...
    public void depositZero() {
        ...
        exception.expectMessage(containsString(
                "Cannot deposit zero amount for account with id " + testAcccount.getId()));
        Account depositedAccount = testAcccount.deposit(Amount.of(BigDecimal.ZERO));
    }

    @Test
    public void withdraw() {
        Account withdrawnAccount = testAcccount.withdraw(Amount.of(new BigDecimal(1)));
        assertThat(withdrawnAccount.getAmount(), equalTo(Amount.of(new BigDecimal(9))));
    }

    @Test
    public void withdrawAll() {
        Account withdrawnAccount = testAcccount.withdraw(Amount.of(new BigDecimal(10)));
    ...
    public void accountWithdrawException() {
        ...
        exception.expectMessage(containsString(
                "Not enough amount to withdraw for account with id " + testAcccount.getId()));
        Account withdrawnAccount = testAcccount.withdraw(Amount.of(new BigDecimal(20)));
        assertThat(withdrawnAccount.getAmount(), equalTo(Amount.of(BigDecimal.ZERO)));
    }
